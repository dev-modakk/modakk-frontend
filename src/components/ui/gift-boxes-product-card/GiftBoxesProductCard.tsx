...
export const GiftBoxesProductCard: React.FC<GiftBoxesProductCardProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-8 sm:mb-12 lg:mb-16">
      {giftBoxes.map((box) => {
        // Normalize the data - handle both API format and mock format
        const isInStock = 'inStock' in box ? box.inStock : !('isSoldOut' in box && box.isSoldOut);
        ...
        return (
          ...
          >
            ...
            <button
              onClick={() => toggleWishlist(String(box.id))}
            ...
            >
              <LuHeart
                className={`w-4 h-4 sm:w-5 sm:h-5 transition-colors duration-200 ${wishlistedItems.includes(String(box.id))
            ...
            <div className="p-4 sm:p-6">
              ...
              <Link href={`/kidsgiftboxes/${String(box.id)}`}>
