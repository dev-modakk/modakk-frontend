...
import { useProductQuery } from '@/query-hooks/product/useProductQuery';
...
export default function ProductPage() {
  ...
  const { data: product, isLoading, error, refetch } = useProductQuery(params.id);

  // Use React Query to fetch related products
  const { data: relatedProducts = [] } = useKidsGiftBoxesHomepage();
  ...
  const related = useMemo(() => {
    if (!product) return relatedProducts.slice(0, 4);
    return relatedProducts.filter((p: any) => p.id !== product.id).slice(0, 4);
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-8">
        <div className="flex items-center gap-3">
          ...
          <span className="text-slate-600 text-lg">Loading product...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-8">
        <div className="text-center">
          <p className="text-2xl font-semibold text-slate-900 mb-2">Unable to load product</p>
          <p className="text-slate-600 mb-6">There was an error loading the product details.</p>
                      ...
                      {product.whatsIncluded?.length ? (
                        <>
                          ...
                          <ul className="list-disc pl-5 text-sm text-slate-700 space-y-1 mb-3">
                            {product.whatsIncluded.map((it: string, i: number) => (
                              <li key={i}>{it}</li>
                            ))}
                          </ul>
                        </>
